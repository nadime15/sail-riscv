...
extern int term_fd; // global from rts

// Global trace log used by riscv_model_impl. Lives here (in the library)
// so any main that links against this library gets it for free.
...
struct Simulator::Impl {
  ...
  bool owns_trace_log = false; // true if we opened a non-stdout trace_log
...
Simulator::Simulator(ModelImpl &model, const SimulatorConfig &cfg) : impl_(std::make_unique<Impl>(model, cfg)) {
  ...
  if (!cfg.trace_log_path.empty()) {
    ...
    if (trace_log == nullptr) {
      trace_log = stdout; // restore so destructor doesn't fclose nullptr
...
RunResult Simulator::run() {
  ...
  while (!m.zhtif_done && (cfg.insn_limit == 0 || impl_->total_insns < cfg.insn_limit)) {
    if (impl_->rvfi.has_value()) {
      ...
      case RVFI_prestep_eof:
        return {RunStatus::RvfiEof, 0, 0, 0};
      case RVFI_prestep_end_trace:
        return {RunStatus::RvfiEndTrace, 0, 0, 0};
...

    {
      ...
      if (m.have_exception) {
        ...
        return {RunStatus::SailException, 0, 0, 0};
      }
      if (cfg.trace_instr) {
        std::fflush(stderr);
        std::fflush(stdout);
    ...
    if (m.zhtif_done) {
      if (m.zhtif_exit_code == 0) {
        return {RunStatus::HtifSuccess, 0, 0, 0};
      } else {
        return {RunStatus::HtifFailure, m.zhtif_exit_code, 0, 0};
      }
    }
  ...
  return {RunStatus::InstructionLimit, 0, 0, 0};
}
...
void Simulator::write_signature() {
  ...
  if (mem_sig_start >= mem_sig_end) {
    std::fprintf(
      stderr,
      "Invalid signature region [0x%" PRIx64 ",0x%" PRIx64 "] to %s.\n",
      mem_sig_start,
      mem_sig_end,
  ...
  for (uint64_t addr = mem_sig_start; addr < mem_sig_end; addr += gran) {
    for (int i = static_cast<int>(gran) - 1; i >= 0; i--) {
      std::fprintf(f, "%02x", static_cast<uint8_t>(read_mem(addr + i)));
    }
    std::fprintf(f, "\n");
  }
  std::fclose(f);
...
void Simulator::reset_for_next_run() {
